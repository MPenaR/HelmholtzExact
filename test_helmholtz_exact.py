...
def test_single_farfield():
    ...
    geo.AddCircle( (x_sc, y_sc), R_sc, leftdomain=scatterer_ID, rightdomain=background_ID, bc="scatterer")
    #geo.AddCircle( (0,0), R_int, leftdomain=background_ID, rightdomain=background_ID, bc="gamma")
    geo.SetMaterial(background_ID,"background")
    geo.SetMaterial(PML_ID,"PML")
    geo.SetMaterial(scatterer_ID,"scatterer")
    ...
    # ax[0].plot(theta_R, FF.real, label="exact")
    # ax[0].plot(theta_R, FF_FEM.real, label="FEM")
    # ax[0].set_title('Real part')
    # ax[0].legend()

    # ax[1].plot(theta_R, FF.imag, label="exact")
    # ax[1].plot(theta_R, FF_FEM.imag, label="FEM")
    # ax[1].set_title('Imaginary part')
    # ax[1].legend()

    # ax[2].plot(theta_R, np.abs(FF), label="exact")
    # ax[2].plot(theta_R, np.abs(FF_FEM), label="FEM")
    # ax[2].set_title('Absolute value')
    # ax[2].legend()
    # plt.show()

    # print(f'{FF.shape=} {FF_FEM.shape=}')
    # print(FF_FEM/FF)

    assert_allclose(FF_FEM, FF, rtol=1E-2)
